...
export const HERO_CTA = { left: px(104), top: py(356), width: pw(218), height: ph(96) }
...
function item(label: string, x: number, y: number, w: number, h: number, img: string): LayoutItem {
  return { label, left: px(x), top: py(y), width: pw(w), height: ph(h), img: F + img }
}

export const NAV_ITEMS: LayoutItem[] = [
  item('My Stuff', 92, 540, 96, 80, 'My_Stuff.png'),
  item('Games', 188, 540, 96, 80, 'Games.png'),
  item('Find', 284, 540, 96, 80, 'Find.png'),
  item('Free', 380, 540, 96, 80, 'Free.png'),
  item('Home', 476, 540, 96, 80, 'Home.png'),
  item('Live', 572, 540, 96, 80, 'Live.png'),
]
...
export const NAV_OVERFLOW = item('More Apps', 1652, 540, 80, 80, 'More_Apps_press_select_to_view_all_of_your_apps_and_channels.png')
export const NAV_SETTINGS = item('Settings', 1740, 540, 80, 80, 'Settings_for_Neal.png')

export const ALL_NAV: LayoutItem[] = [...NAV_ITEMS, ...APP_SHORTCUTS, NAV_OVERFLOW, NAV_SETTINGS]

export interface TileItem { label: string; img: string }
export interface Row { top: number; items: TileItem[] }
...
export const ROW_1: Row = {
  top: 682,
  items: [
    { label: 'Blaze and the Monster Machines', img: F + 'Blaze_and_the_Monster_Machines.png' },
    { label: 'If You Give a Mouse a Cookie', img: F + 'If_You_Give_a_Mouse_a_Cookie.png' },
...
export const ROW_2: Row = {
  ...
  items: [
    ...
    { label: 'Weekend Games', img: F + 'Weekend_Games_-_Jeopardy_Song_Quiz_CoComelon_formerly_Volley.png' },
